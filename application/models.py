...
class Player(Base):
	...
	def set_location(self, location):
		...
		if (type(location) is Room):
			...
			self.world.database_session.add(self)
			self.world.database_session.commit()
			return
		elif(type(location) is int):
			location = self.world.database_session.query(Room).filter_by(id=location).first()
...
class Room(Base):
	...
	def add_exit(self, name, target_room, owner=0):
		...
		if (type(target_room) is int):
			target_room = self.world.database_session.query.query(Room).filter_by(id=target_room).first()
			...
			self.world.database_session.add(self)
			self.world.database_session.add(exit)
			self.world.database_session.commit()
			return
		return
